...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleStaleProduct(StaleProductException ex) {
        log.warn("Conflicto de optimistic locking: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ErrorResponse.of(409, "Conflict", ex.getMessage()));
    }

    @ExceptionHandler({InvalidProductException.class, InvalidCategoryException.class})
    public ResponseEntity<ErrorResponse> handleInvalido(RuntimeException ex) {
    ...
    @ExceptionHandler(ProductCsvFormatException.class)
    public ResponseEntity<ErrorResponse> handleProductCsvFormat(ProductCsvFormatException ex) {
        log.warn("CSV de importación mal formado: {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleValidation(MethodArgumentNotValidException ex) {
        ...
        return ResponseEntity.badRequest()
                .body(ErrorResponse.ofValidation(400, BAD_REQUEST, "Error de validación de campos", fieldErrors));
    }
    ...
    public ResponseEntity<ErrorResponse> handleMethodValidation(HandlerMethodValidationException ex) {
        ...
        List<ErrorResponse.FieldError> fieldErrors = ex.getAllErrors().stream()
                ...
                .toList();
        return ResponseEntity.badRequest()
                .body(ErrorResponse.ofValidation(400, BAD_REQUEST, "Parámetro inválido", fieldErrors));
    }
