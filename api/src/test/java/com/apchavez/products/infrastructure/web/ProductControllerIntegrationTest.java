...
class ProductControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void listInactiveProducts_returnsOnlyInactiveOnes() throws Exception {
        String categoryBody = mockMvc.perform(post("/api/v1/categories")
                        .header("Authorization", "Bearer " + adminToken())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(post("/api/v1/products")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"sku":"CTRL-INACTIVE","name":"Inactive Product","description":"d","categoryId":%d,"price":1.0,"stock":1,"active":false}
                                """.formatted(categoryId)))
                .andExpect(status().isCreated());
        ...
        mockMvc.perform(get("/api/v1/products/inactive")
                        .header("Authorization", "Bearer " + userToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[?(@.sku == 'CTRL-INACTIVE')]").exists());
    }

    @Test
    void searchByNamePrefix_returnsMatchingProducts() throws Exception {
        String categoryBody = mockMvc.perform(post("/api/v1/categories")
                        .header("Authorization", "Bearer " + adminToken())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Search Prefix Category"}"""))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int categoryId = com.jayway.jsonpath.JsonPath.read(categoryBody, "$.id");
        ...
        mockMvc.perform(post("/api/v1/products")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"sku":"CTRL-PREFIX","name":"UniquePrefixWidget","description":"d","categoryId":%d,"price":1.0,"stock":1,"active":true}
                                """.formatted(categoryId)))
                .andExpect(status().isCreated());

        mockMvc.perform(get("/api/v1/products/search")
                        .param("prefix", "UniquePrefix")
                        .header("Authorization", "Bearer " + userToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[?(@.sku == 'CTRL-PREFIX')]").exists());
    }

    @Test
    void searchByCategoryAndPriceRange_returnsMatchingProducts() throws Exception {
        String categoryBody = mockMvc.perform(post("/api/v1/categories")
                        .header("Authorization", "Bearer " + adminToken())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Search Range Category"}"""))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int categoryId = com.jayway.jsonpath.JsonPath.read(categoryBody, "$.id");
        ...
        mockMvc.perform(post("/api/v1/products")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"sku":"CTRL-RANGE","name":"Range Product","description":"d","categoryId":%d,"price":50.0,"stock":1,"active":true}
                                """.formatted(categoryId)))
                .andExpect(status().isCreated());

        mockMvc.perform(get("/api/v1/products/search/by-category-price")
                        .param("categoryId", String.valueOf(categoryId))
                        .param("minPrice", "10")
                        .param("maxPrice", "100")
                        .header("Authorization", "Bearer " + userToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.sku == 'CTRL-RANGE')]").exists());
    }
